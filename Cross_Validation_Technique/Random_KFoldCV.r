...
evaluate_with_k_folds <- function(train_X, train_y, test_X, test_y, validation_X, validation_y, tune_grid, range, dataset, fold_array, scene) {
  results <- data.frame(
    Range = integer(),
    Dataset = integer(),
    Fold = integer(),
  ...
  for (fold in fold_array) {
    ...
    train_control <- trainControl(method = "cv", number = fold, summaryFunction = custom_summary)
  ...
  cat("K FOLD Parallel processing completed and cluster stopped ", range, dataset, scene, "\n")
